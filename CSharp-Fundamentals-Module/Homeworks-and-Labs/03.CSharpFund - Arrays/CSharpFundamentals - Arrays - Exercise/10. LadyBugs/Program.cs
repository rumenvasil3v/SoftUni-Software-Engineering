...
namespace _10._LadyBugs
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            long[] currentIndex = new long[1000];
            ...
            while (input != "end")
            {
                for (int i = 0; i < 1000; i++)
                {
                    ...
                    if (direction == "right")
                    {
                        ...
                        {
                            if (ladyBugIndex + flyLength + flyLength > fieldSize - 1)
                            {
                                currentCells[ladyBugIndex] = 0;
                                input = Console.ReadLine();
                                continue;
                            ...
                            currentCells[ladyBugIndex + flyLength + flyLength] = 1;
                            currentIndex[i] = ladyBugIndex + flyLength + flyLength;
                            currentCells[ladyBugIndex] = 0;
                            input = Console.ReadLine();
                            continue;
                        }
                    ...
                    else if (direction == "left")
                    {
                        ...
                        if (currentCells[ladyBugIndex - flyLength] == 1)
                        {
                            if (ladyBugIndex - flyLength - flyLength < 0)
                            {
                                currentCells[ladyBugIndex] = 0;
                                input = Console.ReadLine();
                                continue;
